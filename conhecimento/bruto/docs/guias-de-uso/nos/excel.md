...
Lê uma planilha Excel (`.xlsx`) e disponibiliza seu conteúdo como dataset. Suporta seleção de aba por nome ou índice, controle de onde começa o cabeçalho e supressão de linhas em branco.
...
> **O formato antigo `.xls` (Excel 97-2003) não é suportado.** O `openpyxl` lê apenas o formato OOXML. Se a sua planilha for `.xls`, abra no Excel e use *Salvar como* → *Pasta de Trabalho do Excel (*.xlsx)*.
