...
| --- | --- | --- | --- |
| [`.env.example`](https://shift.viasoftcloud.com.br/docs/.env.example) | Dev Shift | `docker compose up` na raiz | `.env` |
| [`.env.build.example`](https://shift.viasoftcloud.com.br/docs/.env.build.example) | Cleber / CI | `scripts/build-and-publish.sh` | `.env.build` |
| [`shift-backend/.env.example`](https://shift.viasoftcloud.com.br/docs/shift-backend/.env.example) | Dev Shift | `uvicorn` standalone (sem Docker) | `shift-backend/.env` |
| [`shift-frontend/.env.example`](https://shift.viasoftcloud.com.br/docs/shift-frontend/.env.example) | Dev Shift | `pnpm dev` standalone | `shift-frontend/.env` ou `.env.local` |
| [`shift-mcp-server/.env.example`](https://shift.viasoftcloud.com.br/docs/shift-mcp-server/.env.example) | Cliente final do MCP | Claude Desktop, n8n, etc. | `shift-mcp-server/.env` |
...
**Sou o Cleber e vou publicar uma nova versão manualmente (fora do pipeline):**
→ `.env.build.example`. Copie para `.env.build`, preencha as chaves oficiais (Anthropic, Google, Resend, LangSmith), rode `bash scripts/build-and-publish.sh`. Ver [`BUILD-AND-PUBLISH.md`](https://shift.viasoftcloud.com.br/docs/BUILD-AND-PUBLISH.md).
...
Implementação em [shift-backend/app/core/config.py](https://shift.viasoftcloud.com.br/docs/shift-backend/app/core/config.py).
...
salvou conexões, e depois começou a usar um **servidor novo** apontando para
o **mesmo Postgres**. Sintoma típico:
...
Detalhes em [BUILD-AND-PUBLISH.md](https://shift.viasoftcloud.com.br/docs/BUILD-AND-PUBLISH.md).

### Exceção: o e-mail que os AGENTES enviam

`EMAIL_FROM` continua sendo o remetente **transacional** da plataforma
(convites, OTP, reset de senha) e segue embutido na imagem. Mas o e-mail que a
skill `enviar_email` manda deixou de sair obrigatoriamente por ele: **cada
agente** tem o seu, configurado pelo dono do agente na aba **Poderes → Enviar
email → Remetente**. Três agentes podem usar três contas diferentes.

- **plataforma** — o comportamento antigo (Resend da Shift, cota da Shift);

- **conta Google conectada** — mesma integração do nó de fluxo Gmail; o endereço
é o da conta autenticada, e o `gmail.send` precisa estar na consent screen do
projeto Google;

- **SMTP próprio** — servidor do cliente, senha cifrada em repouso.

Sem configuração, nada muda: continua saindo pela plataforma. A configuração é
guardada por MOLDE (`agent_email_sender`) e salva na hora — não entra no
versionamento do agente, para que reverter uma versão não desconecte a conta.
...
| `SHIFT_MAX_EXECUTION_MEMORY_MB` | 4096 | Mantenha **abaixo** do `mem_limit` do container, senão o OOM kill do Docker age antes do monitor. Prod usa 1536 (`mem_limit: 2g`) |
| `SHIFT_MAX_DISK_GB` | 20 | Servidor com mais disco |
| `SHIFT_NODE_POOL_SIZE` | 16 | Pool de threads de execução de nós, nos dois processos. Prod: 24 no Core, 10 no Worker. Substitui o `SHIFT_THREADPOOL_MAX_WORKERS`, que **nada lê** desde o CIAG-296 |
...
Lista completa em [shift-backend/app/core/config.py](https://shift.viasoftcloud.com.br/docs/shift-backend/app/core/config.py).
